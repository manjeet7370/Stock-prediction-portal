...
class StockPredictionAPIView(APIView):
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            future_days = 30
            temp_input = input_data[-100:].tolist()
            future_predictions = []
           ...
           # 🔥 future line extend
            future_x = range(len(y_test), len(y_test) + len(future_predictions))
            plt.plot(future_x, future_predictions, 'r--', label='Future 30 Days')

            plt.title(f'Prediction + Future Forecast for {ticker}')
            plt.xlabel('Days')
            plt.ylabel('Price')
            plt.legend()
            ...
            today_price = float(np.array(df['Close']).flatten()[-1])
            next_day_price = float(future_predictions[0])
            day_30_price = float(future_predictions[-1])
